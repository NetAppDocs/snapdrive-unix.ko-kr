...
FlexClone 볼륨을 분할하려면 '스냅 연결' 명령으로 '-split' CLI 옵션을 추가로 지정해야 합니다. 연결을 끊으려면 분리 명령이 있는 '-split' 옵션을 제공하여 분할된 FlexClone 볼륨을 연결 해제하도록 해야 합니다. 여기서 분할 동작을 시작하는 블로킹 및 비블로킹 모드가 유효하다.
